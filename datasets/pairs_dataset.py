...
def load_pairs_from_file(fname, root=''):
...
def dnames_to_image_pairs(dnames, data_dir='./data/'):
    ...
    """
    all_pairs = []
    print('Parsing pairs from '+dnames)
    for dname in dnames.split('+'):
        if dname=='habitat_release':
            ...
            pairs = load_pairs_from_file(cache_file, root=dirname)
        print('  {:s}: {:,} pairs'.format(dname, len(pairs)))
        all_pairs += pairs 
    if '+' in dname: print(' Total: {:,} pairs'.format(len(all_pairs)))
    return all_pairs 
